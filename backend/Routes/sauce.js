...
router.post('/', auth, multer, sauceController.createSauce); // Route POST : suit le chemin '/', vérifie le token, traite le fichier image, fait appel à la logique pour création de sauce
...
router.put('/:id', auth, multer, sauceController.modifySauce); // Route PUT : suit le chemin '/:id', vérifie le token, traite le fichier image pour la modification de l'image, fait appel à la logique pour modification de sauce
